...
# jackg@uic.edu
#
# Reads rtr configs, contributes to ipvlanmap, the ip data map of the network, 
...
my $db = 1;  # used to run in database mode

if (! @ARGV)  { 
   print "No argument entered -- argument is <rtr>.cfg filename (no path).\n";
   exit;
}

my $ipvlanmap;  ## hash ref - all ipvlanmap info
my $cfghash;
my $rtr;   ## scalar, holds currently processed router
my $cfg_path = "$installpath/configs/rtr";

my $cfgfile = $ARGV[0];        ## one .cfg file as argument
...
## $rtr is global to the program
($rtr,undef) = split /\./, $cfgfile;
#my $of  = "../forensic/rtr/$rtr.rtrcfgproc";
...
if ($db) {
   my $query    = "DELETE from network.ipvlanmap where context = \"$rtr\"; ";
   ...
   foreach my $ln (@$iv)  {
      #print $ofh "$ln\n";
      my($ip,$vlan,$rtr,$nameif,$nat_type,$outside) = split " ", $ln;
      $insert_h  = $dbh->prepare("INSERT IGNORE into network.ipvlanmap VALUES (?,?,?,?,?,?)");
      $insert_h->execute($ip,$vlan,$rtr,$nameif,$nat_type,$outside);
   }
}
...
sub process_cfg  {
   ...
   foreach my $ln (@$cfg)  {
      ...
      if ($ln =~ /\s*!\s*\z/)  {
         ...
         my $vlan = $interface;
         $vlan =~ s/Vlan//i;
         $cfghash->{$rtr}->{$interface}->{"interface"} = $interface;
         $cfghash->{$rtr}->{$interface}->{"vlan"} = $vlan;
         my (undef,$desc)  = split " ", $tempifh->{"description"}, 2;  ##
         $cfghash->{$rtr}->{$interface}->{"description"} = $desc || "none";
         my (undef,$sec)  = split " ", $tempifh->{"ip vrf forwarding"};
         $cfghash->{$rtr}->{"ip vrf forwarding"} = $sec;
         ## work out all the host addresses from this interface
         $ln = $tempifh->{"ip address"};
         if ($ln =~ /no ip address/)    {
            $cfghash->{$rtr}->{$interface}->{"gateway"} = 0;
            $cfghash->{$rtr}->{$interface}->{"mask"}    = 0;
         }
         else  {
            my (undef,undef,$gateway,$mask) = split " ", $ln;
            $cfghash->{$rtr}->{$interface}->{"gateway"} = $gateway;
            $cfghash->{$rtr}->{$interface}->{"mask"}    = $mask;
            ...
            foreach my $ip ( @{expand_subnet($gateway,$mask)} )  {
               push @{$cfghash->{$rtr}->{$interface}->{"iplist"}}, $ip;
               my $type = "0";  ## not a nat .'. no nat type - interface defined
               $ipvlanmap->{"$ip $vlan $rtr $interface r 0"} = 1;
            }
         }
         %$tempifh = ();  ## go for another interface group
      }
...
#print $ofh "ipvlanmap:\n";
#foreach my $key (sort keys %$ipvlanmap)  { print $ofh "$key\n"; }
