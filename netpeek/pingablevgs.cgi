...
print "<title> pingablevgs: VGs that responded to ping in the previous run </title>";
...
########################################################################
  ...
  my (@swpings,@nopings);
  my $query = "SELECT * FROM switch.vgping";
  ...
  print "<a href=\"#ping\"><b> Pingable VGs </b></a><br><br>\n";
  print "$s4 Note: Some VGs are merely pingable, and may not answer in any other way. <br><br>\n";
  print "<a href=\"#non\"><b> NON-Pingable VGS </b></a><br><br>\n";
  print "$s4 Note: Some of these may exist only as DNS entries. <br><br>\n";

  print "<hr><br>";
  print "<a name=\"ping\"><b> Current pingable VGs on UIC networks </b></a><br><br>\n";
  print "<table border = 5>";
  print " <th> VG name </th> <th> VG ip </th>"; 
  foreach my $sw (@swpings)  { 
     my ($swname,$swip) = split " ", $sw;
     print "<tr> <td> <a href=\"netpeek/netpeek.cgi?submit=Submit&oper=query_ip&ip=$swip\" $blank > $swname </a> $s2 </td>"; 
     print "<td> $swip </td>";
  }
  print "</table> <br>";

  print "<hr><br>";
  print "<a name=\"non\" ><b>Current NON-pingable VGs on UIC networks </b></a><br><br>\n";
  print "<table border=5> ";
  print "<th> VG IP  </th> <th> VG name </th>";
